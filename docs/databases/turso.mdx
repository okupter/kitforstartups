...
```bash
# Create a src/lib/server/drizzle/turso/data folder
turso dev -f src/lib/server/drizzle/turso/data/data.sqlite
```
...
If you are using VSCode, you can exclude the `src/lib/server/drizzle` and `src/lib/server/lucia` folders from the search.
