...
```bash
turso dev -f src/lib/server/db/turso/data.sqlite
```
...
If you are using VSCode, you can exclude the `src/lib/server/db` and `src/lib/server/auth/lucia` folders from the search.
