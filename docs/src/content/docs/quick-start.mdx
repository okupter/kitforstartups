...
The default configuration for KitForStartups uses SQLite as a database through [Turso](https://turso.tech).
...
<Aside>Remember to update the MYSQL\_\* environment variables in the `.env` file with your database credentials.</Aside>
...
This will generate the SQL migrations, as well as migrations metadata files in the `./src/lib/server/db/mysql/migrations/data` directory.
...
<Aside> Remember to update the TRANSACTIONAL\_EMAILS\_\* environment variables in the `.env` file with your sender name and address.</Aside>
...
- Change occurences of `$lib/server/auth/lucia/turso` to `$lib/server/auth/lucia/postgres` in the `./src` directory
- Change occurences of `$lib/server/db/mysql/models` to `$lib/server/db/postgres/models` in the `./src` directory
